...
- **Procure inconsistência entre camadas, não só vulnerabilidade por classe.** Os furos de maior valor moram onde duas representações do mesmo sistema **discordam**: dois escritores da mesma tabela com regras diferentes, o front valida o que a API aceita, o OpenAPI declara o que o runtime não cumpre, um header afirma o que o comportamento contradiz, a doc promete o que o código não faz. Para cada dado/estado sensível pergunte *"quem mais toca isto, e concordam?"* — a **contradição** é o achado, e nenhuma varredura classe-a-classe (que olha cada rota isolada) a enxerga. É uma classe de busca própria, não um item de checklist.
...
3. **Rode o ferramental antes de ler à mão.** Triagem automática primeiro; grep e leitura depois, para confirmar e achar o que a ferramenta não pega.
   ...
   - **Runner de schema/RLS drift (camada de dados):** `py tools/schema_drift.py --dsn postgres://... [--expect expect.json]` (só-leitura) — o par de banco do `scan.py`. Pega o que o whitebox de código NÃO vê: RLS desligada em produção, grant para `anon`, policy `USING (true)`, coluna sensível gravável fora da whitelist. É deriva entre o que o código assume e o que o banco aplica — a única classe onde um modelo forte não ajuda (é fato de prod ausente do contexto, não raciocínio). Detalhe e formato do `--expect` em `references/supabase-rls.md`.
...
4. **Identifique pontos fracos — por classe E por inconsistência.** Varra classe por classe de falha, por ponto de entrada, usando o arquivo de referência da stack. **Depois cruze as camadas:** liste quem lê e quem *escreve* cada dado/estado sensível e verifique se concordam na regra (whitelist, authz, validação, estado válido). Divergência entre dois caminhos para o mesmo recurso é achado — e é invisível a quem audita rota a rota.
...
8. **Passe pelo Portão de validação (advogado do diabo formalizado).** Para cada achado ≥Médio, rode as 6 perguntas do **Portão de validação** (`references/owasp-web.md`): alcançável? sem mitigação no caminho? cenário concreto? contraprova falhou? severidade contextual? fato ou hipótese? Falhou uma → rebaixa ou vira "A verificar", não entra como Confirmado. Obrigatório — é a diferença entre relatório e ruído.
   - **[Experimental] Refutação independente para Crítico/Alto.** O advogado do diabo acima é *você mesmo* — já viu sua conclusão, então tende a confirmá-la (viés de confirmação, fechamento prematuro). Para cada achado Crítico/Alto, quando houver orçamento, submeta-o a uma **segunda mente cega à sua conclusão**: um subagente que recebe só o código + a afirmação neutra (*"X é explorável? Tente derrubar."*) — **não** o seu raciocínio. Sobreviveu à refutação → confiança sobe; caiu → rebaixa ou "A verificar". É a única forma de quebrar o viés estruturalmente, não por auto-instrução. Custo zero de prompt (usa subagente); pula em diff trivial.
